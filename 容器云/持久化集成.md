...
node{
    ...
    stage('maven build'){
        sh '''/usr/local/maven/bin/mvn package -DskipTests -f var/jenkins_home/workspace/ChinaskillProject'''    #mvn 打包项目
    }
    stage('image build'){
        sh '''
    ...
    stage('deploy Rancher'){
        //执行部署脚本
       sh 'sed -i "s/sqshq\\/piggymetrics-gateway/10.10.16.10\\/chinaskillproject\\/gateway:$BUILD_ID/g" /var/jenkins_home/workspace/ChinaskillProject/yaml/deployment/gateway-deployment.yaml'          #改镜像名字，因为node节点上没有此镜像，这两步可以直接改yaml文件
