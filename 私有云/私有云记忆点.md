...
```bash
1. 迁移虚拟机目录 scp -r 
/var/lib/nova/instances/中对应的虚拟机目录，删除compute目录
2. 修改数据库文件
nova库中instances表对应虚拟机的host和node字段改成另一台主机名  
select * from  instances\G;  查看uuid    
...
```bash
当前全是用的是SSD硬盘，那么none算法更合适，使用提供的OpenStack私有云平台，优化KVM的I/O调度算法，将默认的deadline修改为none模式
...
```
使用 iaas-install-cloudkitty.sh 脚本安装 cloudkitty 服务，安装完毕后，启用 hashmap 评级 模块，接着创建 volume_thresholds 组，创建服务匹配规则 volume.size，并设置每 GB 的价 格为 0.01。接下来对应大量数据设置应用折扣，在组 volume_thresholds 中创建阈值，设置 若超过 50GB 的阈值，应用 2%的折扣（0.98）。
#0.启用hashmap
...
# compute
pvcreate /dev/sdc       #将物理硬盘分区初始化为物理卷
vgextend cinder-volumes /dev/sdc   #动态扩展LVM卷组
vgdisplay      #查看
...
# --display-name 这是什么意思
--name 可以不写，也可以写成--display-name 名字
```

manila共享文件系统服务（创建类型有什么用？）127.0.0.0/24 允许共享ip地址写这个
...
[Service]
LimitNOFILE=50000     #记住

systemctl daemon-reload
systemctl restart rabbitmq-server    #必须写rabbitmq-server 因为上面是rabbitmq-server
...
openstack image save 1c7d620b-0f75-4f1f-9dd0-5ee355ac51cf  --file /root/cloudsave/csccvm.qcow2 
